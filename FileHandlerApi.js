...
function PickAttachmentMetaData(UUID, Company, Tenant, reqId, callback) {
  try {
    if (UUID) {
      try {
        DbConn.FileUpload.find({
          ...
          .then(function (resFile) {
            if (resFile) {
              logger.debug(
                "[DVP-FIleService.PickAttachmentMetaData] - [%s] - [PGSQL] - Uploaded file %s  metadata found ",
                reqId,
                UUID
              );
              ...
              logger.error(
                "[DVP-FIleService.PickAttachmentMetaData] - [%s] - [PGSQL] - Uploaded file %s metadata not found ",
                reqId,
                UUID
              );
              callback(
                new Error("No record found for id : " + UUID),
                undefined
              );
            }
          })
          .catch(function (errFile) {
            logger.error(
              "[DVP-FIleService.PickAttachmentMetaData] - [%s] - [PGSQL] - Error occurred while searching for Uploaded file Metadata %s  ",
              reqId,
              UUID
            );
        ...
        logger.error(
          "[DVP-FIleService.PickAttachmentMetaData] - [%s] - Exception occurred when starting PickAttachmentMetaData %s ",
          reqId,
          UUID
        );
      ...
      logger.error(
        "[DVP-FIleService.PickAttachmentMetaData] - [%s] - Invalid Input for UUID %s",
        reqId,
        UUID
      );
    ...
    logger.error(
      "[DVP-FIleService.PickAttachmentMetaData] - [%s] - Exception in operation / File ID: %s",
      reqId,
      UUID
    );
...
function PickAttachmentMetaDataByName(
  FileName,
  Company,
  Tenant,
  reqId,
  ...
  try {
    if (FileName) {
      try {
        logger.debug(
          "[DVP-FIleService.PickAttachmentMetaDataByName] - [%s] - Searching for Uploaded file %s",
          reqId,
          FileName
        );

        DbConn.FileUpload.max("Version", {
          where: [
            { Filename: FileName },
            { CompanyId: Company },
            { TenantId: Tenant },
          ...
          .then(function (resMax) {
            if (resMax) {
              logger.debug(
                "[DVP-FIleService.PickAttachmentMetaDataByName] - [%s] - Max version found for file %s",
                reqId,
                FileName
              );
              ...
              DbConn.FileUpload.find({
                ...
                .then(function (resUpFile) {
                  if (resUpFile) {
                    logger.debug(
                      "[DVP-FIleService.PickAttachmentMetaDataByName] - [%s] - Fie found",
                      reqId,
                      FileName
                    );
                    ...
                    logger.error(
                      "[DVP-FIleService.PickAttachmentMetaDataByName] - [%s] - No such file found",
                      reqId,
                      FileName
                    );
                    callback(undefined, resUpFile);
                ...
                .catch(function (errFile) {
                  logger.error(
                    "[DVP-FIleService.PickAttachmentMetaDataByName] - [%s] - Error in searching files",
                    reqId,
                    FileName
                  );
                  callback(errFile, undefined);
              ...
              logger.error(
                "[DVP-FIleService.PickAttachmentMetaDataByName] - [%s] - No version found ",
                reqId,
                FileName
              );
          ...
          .catch(function (errMax) {
            logger.error(
              "[DVP-FIleService.PickAttachmentMetaDataByName] - [%s] - Error in searching max version ",
              reqId,
              FileName
            );
      ...
      logger.error(
        "[DVP-FIleService.PickAttachmentMetaData] - [%s] - Invalid Input for FileName %s",
        reqId,
        FileName
      );
    ...
    logger.error(
      "[DVP-FIleService.PickAttachmentMetaData] - [%s] - Exception in operation %s",
      reqId,
      FileName
    );
    callback(e, undefined);
...
function LocalFileDownloader(fileObj, res) {
  logger.debug(
    "[DVP-FIleService.DownloadFile] - [%s] - [PGSQL] - File record found  %s",
    fileObj.reqId,
    fileObj.id
  );
  try {
    ...
    if (!fileObj.URL) {
      logger.error(
        "[DVP-FIleService.DownloadFile] - [%s] - [FILEDOWNLOAD] - No File path found",
        fileObj.reqId
      );
      res.status(400);
      res.end();
    ...
    logger.debug(
      "[DVP-FIleService.DownloadFile] - [%s]  - [FILEDOWNLOAD] - SourcePath of file %s",
      fileObj.reqId,
      SourcePath
    );

    logger.debug(
      "[DVP-FIleService.DownloadFile] - [%s]  - [FILEDOWNLOAD] - ReadStream is starting",
      fileObj.reqId
    );
    ...
    if (fileObj.isEncryptedFile && fileObj.method == "DEFAULT") {
      ...
      source
        .on("end", function (result) {
          logger.debug(
            "[DVP-FIleService.DownloadFile] - [%s] - [FILEDOWNLOAD] - Piping succeeded",
            fileObj.reqId
          );
          decrypt.end();
        })
        .on("error", function (err) {
          logger.error(
            "[DVP-FIleService.DownloadFile] - [%s] - [FILEDOWNLOAD] - Error in Piping",
            fileObj.reqId,
            err
          );
          decrypt.end();
    ...
    logger.error(
      "[DVP-FIleService.DownloadFile] - [%s] - [FILEDOWNLOAD] - Exception occurred when download section starts",
      fileObj.reqId,
      ex
    );
...
function DownloadFileByID(res, fileObj) {
  ...
  if (fileObj.id) {
    try {
      logger.debug(
        "[DVP-FIleService.DownloadFile] - [%s] - Searching for Uploaded file %s",
        fileObj.reqId,
        fileObj.id
      );
      DbConn.FileUpload.find({
        ...
        include: [{ model: DbConn.FileCategory, as: "FileCategory" }],
      })
        .then(function (resUpFile) {
          if (resUpFile) {
            var resObj = {
              ...
              ETag:
                resUpFile.UniqueId + ":" + "display" + ":" + resUpFile.Version,
            };
            ...
            if (readyToDownload) {
              ...
              logger.error(
                "[DVP-FIleService.DownloadFile] - [%s] - [FILEDOWNLOAD] - Unauthorized to download",
                fileObj.reqId
              );
            ...
            logger.error(
              "[DVP-FIleService.DownloadFile] - [%s] - [PGSQL] - No record found for  Uploaded file  %s",
              fileObj.reqId,
              fileObj.id
            );
            res.status(404);
        ...
        .catch(function (errUpFile) {
          logger.error(
            "[DVP-FIleService.DownloadFile] - [%s] - [PGSQL] - Error occurred while searching Uploaded file  %s",
            fileObj.reqId,
            fileObj.id,
            errUpFile
          );
      ...
      logger.error(
        "[DVP-FIleService.DownloadFile] - [%s] - [FILEDOWNLOAD] - Exception occurred while starting File download service",
        fileObj.reqId,
        fileObj.id
      );
      res.status(400);
    ...
    logger.error(
      "[DVP-FIleService.DownloadFile] - [%s] - [FILEDOWNLOAD] - No record found for UUID %s",
      fileObj.reqId
    );
    res.status(404);
    res.end();
...
function FileInfoByID(res, UUID, Company, Tenant, reqId) {
  logger.debug(
    "[DVP-FIleService.FileInfoByID] - [%s] - Searching for Uploaded file %s",
    reqId,
    UUID
  );
  try {
    if (UUID) {
      DbConn.FileUpload.find({
        ...
        .then(function (resFile) {
          if (resFile) {
            ...
            logger.debug(
              "[DVP-FIleService.FileInfoByID] - [%s] - No such file found for ID %s",
              reqId,
              UUID
            );
            res.status(404);
        ...
        .catch(function (errFile) {
          logger.error(
            "[DVP-FIleService.FileInfoByID] - [%s] - Error in searching records for ID  %s",
            reqId,
            UUID,
            errFile
          );
      ...
      logger.error(
        "[DVP-FIleService.FileInfoByID] - [%s] - Invalid ID  %s",
        reqId,
        UUID
      );
      res.status(404);
    ...
    logger.error(
      "[DVP-FIleService.FileInfoByID] - [%s] - Exception in operation",
      reqId
    );
    res.status(400);
    res.end();
...
function DownloadLatestFileByID(res, fileObj) {
  try {
    logger.debug(
      "[DVP-FIleService.DownloadLatestFileByID] - [%s] - Searching for Uploaded file %s",
      fileObj.reqId,
      fileObj.FileName
    );
    ...
    DbConn.FileUpload.max("Version", { where: [conditionalData] })
      .then(function (resMax) {
        if (resMax) {
          logger.debug(
            "[DVP-FIleService.DownloadLatestFileByID] - [%s] - Max version found for file %s",
            fileObj.reqId,
            fileObj.FileName
          );

          DbConn.FileUpload.findOne({
            where: [
              { CompanyId: fileObj.Company },
              { TenantId: fileObj.Tenant },
              { Filename: fileObj.FileName },
            ...
            .then(function (resUpFile) {
              if (resUpFile) {
                ...
                logger.debug(
                  "[DVP-FIleService.DownloadLatestFileByID] - [%s] - ID found of file %s  ID : %s ",
                  fileObj.reqId,
                  fileObj.FileName,
                  UUID
                );
                ...
                if (fileStore == "MONGO") {
                  logger.debug(
                    "[DVP-FIleService.DownloadLatestFileByID] - [%s] - [MONGO] - Downloading from Mongo",
                    fileObj.reqId,
                    JSON.stringify(resUpFile)
                  );
                  MongoFileDownloader(
                    UUID,
                    isEncryptedFile,
                  ...
                  logger.debug(
                    "[DVP-FIleService.DownloadLatestFileByID] - [%s] - [PGSQL] - Record found for File upload %s",
                    fileObj.reqId,
                    JSON.stringify(resUpFile)
                  );
                  LocalFileDownloader(dataObj, res);
                ...
                logger.error(
                  "[DVP-FIleService.DownloadLatestFileByID] - [%s] - No such file found",
                  fileObj.reqId,
                  FileName
                );
                res.status(404);
            ...
            .catch(function (errFile) {
              logger.error(
                "[DVP-FIleService.DownloadLatestFileByID] - [%s] - Error in file searching",
                fileObj.reqId,
                errFile
              );
              res.status(400);
          ...
          logger.error(
            "[DVP-FIleService.DownloadLatestFileByID] - [%s] - Max not found",
            fileObj.reqId
          );
          res.status(404);
          res.end();
      ...
      .catch(function (errMax) {
        logger.error(
          "[DVP-FIleService.DownloadLatestFileByID] - [%s] - Error in Max",
          fileObj.reqId,
          errMax
        );
        res.status(400);
    ...
    logger.error(
      "[DVP-FIleService.DownloadLatestFileByID] - [%s] - [FILEDOWNLOAD] - Exception occurred while starting File download service",
      reqId,
      FileName
    );
...
function LatestFileInfoByID(res, FileName, Company, Tenant, reqId) {
  try {
    logger.debug(
      "[DVP-FIleService.LatestFileInfoByID] - [%s] - Searching for Uploaded file %s",
      reqId,
      FileName
    );
    ...
    DbConn.FileUpload.max("Version", {
        ...
        if (resMax) {
          DbConn.FileUpload.findOne({
            ...
            .then(function (resUpFile) {
              if (resUpFile) {
                logger.debug(
                  "[DVP-FIleService.LatestFileInfoByID] - [%s] - File found FileName %s",
                  reqId,
                  FileName
                );
                ...
                logger.error(
                  "[DVP-FIleService.LatestFileInfoByID] - [%s] - File not found FileName %s",
                  reqId,
                  FileName
                );
                res.status(404);
            ...
            .catch(function (errFile) {
              logger.error(
                "[DVP-FIleService.LatestFileInfoByID] - [%s] - Error in file searching FileName %s",
                reqId,
                FileName,
                errFile
              );
      ...
      .catch(function (errMax) {
        logger.error(
          "[DVP-FIleService.LatestFileInfoByID] - [%s] - Error in searching Latest File , FileName %s",
          reqId,
          FileName,
          errMax
        );
    ...
    logger.error(
      "[DVP-FIleService.LatestFileInfoByID] - [%s] - [FILEDOWNLOAD] - Exception occurred while starting File download service %s ",
      reqId,
      FileName
    );
    //callback(new Error("No record Found for the request"), undefined);
...
function PickVoiceClipByName(
  ...
  try {
    if (FileName && AppID && !isNaN(AppID)) {
                  ...
                  if (resVersion) {
                    DbConn.FileUpload.find({
                      ...
                      .then(function (resFile) {
                        if (resFile) {
                          logger.debug(
                            "[DVP-FIleService.PickVoiceClipByName] - [%s] - [PGSQL] - Record found for Application %s  result ",
                            reqId,
                            AppID
                          );
                          callback(undefined, resFile);
                          ...
                          logger.error(
                            "[DVP-FIleService.PickVoiceClipByName] - [%s] - [PGSQL] - No record found for Application %s  ",
                            reqId,
                            AppID
                          );
                          callback(
                            new Error("No record found for Application"),
                            undefined
                          );
                        }
                      })
                      .catch(function (errFile) {
                        logger.error(
                          "[DVP-FIleService.PickVoiceClipByName] - [%s] - [PGSQL] - Error occurred while searching for Application %s  ",
                          reqId,
                          AppID,
                          errFile
                        );
      ...
      logger.error(
        "[DVP-FIleService.PickVoiceClipByName] - [%s] - [PGSQL] - Invalid inputs found  ",
        reqId
      );
    ...
    logger.error(
      "[DVP-FIleService.PickVoiceClipByName] - [%s] - [PGSQL] - Exception in operation  ",
      reqId
    );
    callback(e, undefined);
  }
...
function CurrentFileVersion(Company, Tenant, AppID, FileName, reqId, callback) {
  try {
    logger.debug(
      "[DVP-FIleService.DeveloperUploadFiles.FindCurrentVersion] - [%s] - Searching for current version  File of %s",
      reqId,
      FileName
    );
    ...
    DbConn.FileUpload.max("Version", {
      ...
      .then(function (resMax) {
        if (resMax) {
          logger.debug(
            "[DVP-FIleService.DeveloperUploadFiles.FindCurrentVersion] - [%s] - [PGSQL] - Old version of %s is found and New version updated",
            reqId,
            FileName
          );
          ...
          logger.debug(
            "[DVP-FIleService.DeveloperUploadFiles.FindCurrentVersion] - [%s] - [PGSQL] -  Version of %s is not found and New version will be %s",
            reqId,
            FileName,
            "1"
          );
      ...
      .catch(function (errMax) {
        logger.error(
          "[DVP-FIleService.DeveloperUploadFiles.FindCurrentVersion] - [%s] - [PGSQL] - Error occurred while searching for current version of %s",
          reqId,
          FileName,
          errMax
        );
    ...
    logger.error(
      "[DVP-FIleService.DeveloperUploadFiles.FindCurrentVersion] - [%s] - Exception occurred when start searching current version of %s",
      reqId,
      ex
    );
    callback(ex, undefined);
...
function PickFileInfo(appid, Company, Tenant, reqId, callback) {
  try {
    if (appid && !isNaN(appid)) {
      try {
        DbConn.FileUpload.findAll({
          ...
          .then(function (resFile) {
            if (!resFile) {
              logger.error(
                "[DVP-FIleService.PickFileInfo] - [%s] - [PGSQL] - No files found for Application %s  ",
                reqId,
                appid
              );
              ...
              logger.info(
                "[DVP-FIleService.PickFileInfo] - [%s] - [PGSQL] - Files found for Application %s  ",
                reqId,
                appid
              );
              callback(undefined, resFile);
          ...
          .catch(function (errFile) {
            logger.error(
              "[DVP-FIleService.PickFileInfo] - [%s] - [PGSQL] - Error in searching files for Application %s  ",
              reqId,
              appid,
              errFile
            );
        ...
        logger.error(
          "[DVP-FIleService.PickFileInfo] - [%s] - [PGSQL] - Exception in searching files found for Application %s  ",
          reqId,
          appid
        );
        callback(ex, undefined);
      ...
      logger.error(
        "[DVP-FIleService.PickFileInfo] - [%s] - [PGSQL] -Invalid application ID found  ",
        reqId
      );
    ...
    logger.error(
      "[DVP-FIleService.PickFileInfo] - [%s] - [PGSQL] -Exception in opeation  ",
      reqId
    );
    callback(e, undefined);
  }
...
function PickFileWithAppID(UUID, appid, Company, Tenant, reqId, callback) {
  try {
    if (UUID && appid && !isNaN(appid)) {
      try {
        DbConn.FileUpload.find({
          ...
          .then(function (resFile) {
            if (!resFile) {
              logger.error(
                "[DVP-FIleService.PickFileWithAppID] - [%s] - [PGSQL] - No file found for ID %s ",
                reqId,
                UUID
              );
              callback(new Error("No file"), undefined);
              ...
              logger.info(
                "[DVP-FIleService.PickFileWithAppID] - [%s] - [PGSQL] - File found for ID %s ",
                reqId,
                UUID
              );
              callback(undefined, resFile);
          ...
          .catch(function (errFile) {
            logger.error(
              "[DVP-FIleService.PickFileWithAppID] - [%s] - [PGSQL] - Error in searching file found for ID %s ",
              reqId,
              UUID,
              errFile
            );
        ...
        logger.error(
          "[DVP-FIleService.PickFileWithAppID] - [%s] - [PGSQL] - Exception in searching file found for ID %s ",
          reqId,
          UUID,
          ex
        );
      ...
      logger.error(
        "[DVP-FIleService.PickFileWithAppID] - [%s] - [PGSQL] - Invalid AppID or File ID found ",
        reqId
      );
    ...
    logger.error(
      "[DVP-FIleService.PickFileWithAppID] - [%s] - [PGSQL] - Exception in operation ",
      reqId,
      e
    );
    callback(e, undefined);
...
function PickAllVoiceRecordingsOfSession(
  SessID,
  Company,
  Tenant,
  reqId,
  ...
  try {
    DbConn.FileUpload.findAll({
      where: [{ RefId: SessID }, { CompanyId: Company }, { TenantId: Tenant }],
    })
...
function AllVoiceRecordingsOfSessionAndTypes(
  SessID,
  Class,
  Type,
  Category,
...
function FilesWithCategoryAndDateRange(
  CategoryID,
  Company,
  Tenant,
  startDate,
...
function FilesWithCategoryListAndDateRange(
  req,
  Company,
  Tenant,
  startDate,
...
function FilesWithCategoryListAndDateRangeCount(
  req,
  Company,
  Tenant,
  startDate,
...
function AllFilesWithCategoryAndDateRange(
  Category,
  Company,
  Tenant,
  startDate,
...
function AllFilesWithCategoryAndDateRangeCount(
  Category,
  Company,
  Tenant,
  startDate,
...
function AllFilesWithCategoryID(
  CategoryID,
  rowCount,
  pageNo,
  Company,
  ...
  try {
    DbConn.FileUpload.findAll({
      ...
      limit: rowCount,
      include: [
        { model: DbConn.FileCategory, as: "FileCategory" },
        { model: DbConn.Application, as: "Application" },
      ],
...
function PickFilesByCategoryList(
  rowCount,
  pageNo,
  Company,
  Tenant,
...
function PickCategorySpecifiedFiles(
  fileCategory,
  fileFormat,
  Company,
  Tenant,
...
function PickAllFilesWithPaging(
  rowCount,
  pageNo,
  Company,
  Tenant,
        ...
        if (exists) {
          ...
          fs.unlink(URL, function (err) {
            if (err) {
              ...
              RedisPublisher.UpdateFileStorageRecords(
                "RELEASE",
                resFile.ObjCategory,
                resFile.Size,
                Company,
              ...
              if (
                resFile.FileStructure &&
                resFile.FileStructure.split("/")[0] == "image"
              ) {
                var thumbDir = path.join(
                  uploadPath,
                  "Company_" +
                    Company.toString() +
                    "_Tenant_" +
                    Tenant.toString(),
                  resFile.ObjCategory + "_thumb",
                  year.toString() +
                    "-" +
                    month.toString() +
                    "-" +
                ...
                fs.unlink(
                  path.join(thumbDir, (resFile.UniqueId + "_75").toString())
                );
                fs.unlink(
                  path.join(thumbDir, (resFile.UniqueId + "_100").toString())
                );
                fs.unlink(
                  path.join(thumbDir, (resFile.UniqueId + "_125").toString())
                );
                fs.unlink(
                  path.join(thumbDir, (resFile.UniqueId + "_150").toString())
                );
                fs.unlink(
                  path.join(thumbDir, (resFile.UniqueId + "_200").toString())
                );
              } else {
...
function DeleteFile(fileID, Company, Tenant, option, reqId, callback) {
  try {
    PickAttachmentMetaData(
      fileID,
      Company,
      Tenant,
      reqId,
      function (errFile, resFile) {
        if (errFile) {
          ...
          if (resFile) {
            ...
            callback(
              new Error("No File record found to delete " + fileID),
              undefined
            );
          }
...
function SaveCategoryBulk(reqId, categoryDataList, company, tenant, callback) {
  try {
    ...
    DbConn.FileCategory.bulkCreate(newCatList)
    ...
    logger.error(
      "[DVP-FIleService.SaveNewCategory] - [%s] - [FILECATEGORY] - Exception occurred while creating file category",
      reqId,
      ex
    );
    callback(ex, null);
  ...
  try {
    DbConn.FileCategory.findOne({
      ...
      .then(function (resCat) {
        if (resCat) {
          logger.error(
            "[DVP-FIleService.SaveNewCategory] - [%s] - [PGSQL] -  File category %s is already exists for your company",
            reqId,
          ...
          callback(
            new Error(
              "File category " +
                categoryData.Category +
                "is already exists for your company"
            ),
            undefined
          );
        } else {
          var CatObject = DbConn.FileCategory.build({
            Category: categoryData.Category,
            Owner: "user",
          ...
          CatObject.save()
            .then(function (resSave) {
              logger.info(
                "[DVP-FIleService.SaveNewCategory] - [%s] - [PGSQL] - Save new file category succeeded %s",
                reqId,
                resSave
              );
            ...
            .catch(function (errSave) {
              logger.error(
                "[DVP-FIleService.SaveNewCategory] - [%s] - [PGSQL] - Error occurred while saving file category %s",
                reqId,
      ...
      .catch(function (errCat) {
        logger.error(
          "[DVP-FIleService.SaveNewCategory] - [%s] - [PGSQL] -  File category %s is validating failed",
          reqId,
          categoryData.Category
        );
...
function LoadCategories(reqId, company, tenant, isAll, callback) {
  try {
    ...
    DbConn.FileCategory.findAll({ where: qObj })
      .then(function (resFile) {
        callback(undefined, resFile);
      })
      .catch(function (errFile) {
...
function PickFileCountsOFCategories(catID, company, tenant, callback) {
  DbConn.FileCategory.find({ where: [{ id: catID }] })
    .then(function (resCat) {
      if (!resCat) {
        ...
        DbConn.FileUpload.count({
          where: [
            { FileCategoryId: catID.toString() },
            { CompanyId: company },
            { TenantId: tenant },
          ],
        })
          ...
          overdue_done: {
            $sum: {
              $cond: [
                {
                  $and: [
                    { $gt: ["$ticket_matrix.solved_at", "$due_at"] },
                    { $eq: ["$status", "closed"] },
                  ],
                },
                1,
...
function updateFilePath(req, res) {
  ...
  logger.debug(
    "[DVP-FIleService.updateFilePath] - [%s] - [HTTP] - Request received - Inputs - File ID : %s ",
    reqId,
    req.params.id
  );
  ...
  if (req.params.id && req.body.URL && req.body.Source) {
    try {
      DbConn.FileUpload.findOne({
        ...
        .then(function (resFile) {
          if (resFile) {
            resFile
              .updateAttributes({ URL: req.body.URL, Source: req.body.Source })
              .then(function (resUpdate) {
                if (resUpdate) {
                  logger.debug(
                    "[DVP-FIleService.updateFilePath] - [%s] - [PGSQL] - URL of File %s updated successfully ",
                    reqId,
                    req.params.id
                  );
                  var jsonString = messageFormatter.FormatMessage(
                    undefined,
                    "URL of File " + req.params.id + " updated successfully",
                    true,
                    resUpdate
                  );
                  res.end(jsonString);
                  ...
                  logger.debug(
                    "[DVP-FIleService.updateFilePath] - [%s] - [PGSQL] - URL of File %s updating of failed ",
                    reqId,
                    req.params.id
                  );
                  var jsonString = messageFormatter.FormatMessage(
                    new Error(
                      "URL updating of File " + req.params.id + " failed"
                    ),
                    "URL updating of File " + req.params.id + " failed",
                    false,
                    undefined
            ...
            logger.debug(
              "[DVP-FIleService.updateFilePath] - [%s] - [PGSQL] - No file record found for ID",
              reqId,
              req.params.id
            );
            var jsonString = messageFormatter.FormatMessage(
              new Error("No file record found for ID " + req.params.id),
              "No file record found for ID " + req.params.id,
              false,
              undefined
            );
            res.end(jsonString);
        ...
        .catch(function (errFile) {
          logger.debug(
            "[DVP-FIleService.updateFilePath] - [%s] - [PGSQL] - No File found for ID  %s  ",
            reqId,
            req.params.id
          );
          var jsonString = messageFormatter.FormatMessage(
            errFile,
            "No File found for ID " + req.params.id,
            false,
            undefined
          );
          res.end(jsonString);
      ...
      logger.debug(
        "[DVP-FIleService.updateFilePath] - [%s] - [PGSQL] - Error in operation  ",
        req.params.id
      );
      var jsonString = messageFormatter.FormatMessage(
        e,
    ...
    logger.debug(
      "[DVP-FIleService.updateFilePath] - [%s] - [PGSQL] - Insufficient  received  ",
      req.params.id
    );
    var jsonString = messageFormatter.FormatMessage(
      new Error(" Insufficient  received"),
...
function UpdateCategory(catId, body, company, tenant, callback) {
  ...
  DbConn.FileCategory.findOne({
    where: [{ id: catId }, { Company: company }, { Tenant: tenant }],
  })
    .then(function (resCat) {
      resCat
