...
function FindCurrentVersion(fname, company, tenant, reqId, Category, callback) {
  try {
    logger.debug(
      "[DVP-FIleService.FindCurrentVersion.FindCurrentVersion] - [%s] - Searching for current version of %s",
      reqId,
      fname
    );
    DbConn.FileUpload.max("Version", {
      ...
      .then(function (resFile) {
        if (resFile) {
          ...
          if (resFile < 0) {
            logger.debug(
              "[DVP-FIleService.FindCurrentVersion.FindCurrentVersion] - [%s] - [PGSQL] -  Reserved file found",
              reqId,
              fname
            );
            ...
            logger.debug(
              "[DVP-FIleService.FindCurrentVersion.FindCurrentVersion] - [%s] - [PGSQL] -  Version of %s is not found and New version will be 1",
              reqId,
              fname
            );
      ...
      .catch(function (errFile) {
        logger.error(
          "[DVP-FIleService.FindCurrentVersion.FindCurrentVersion] - [%s] - [PGSQL] - Error occurred while searching for current version of %s",
          reqId,
          fname,
          errFile
        );
    ...
    logger.error(
      "[DVP-FIleService.FindCurrentVersion.FindCurrentVersion] - [%s] - Exception occurred when start searching current version of %s",
      reqId,
      FObj.name,
      ex
    );
...
function LocalThumbnailMaker(thumObj, callback) {
  ...
  try {
    if (fileStruct == "image") {
      logger.info(
        "[DVP-FIleService.LocalThumbnailMaker] - [%s] - Image file found to make thumbnails ",
        thumObj.reqId,
        thumObj.uuid
      );
      mkdirp(thumObj.thumbDir, function (err) {
        if (err) {
          logger.error(
            "[DVP-FIleService.LocalThumbnailMaker] - [%s] - Error occurred in making directory for thumbnails of %s ",
            reqId,
            uuid
          );
          ...
          sizeArray.forEach(function (size) {
            thumbnailArray.push(function createContact(callbackThumb) {
              ...
              var writeStream = fs.createWriteStream(
                path.join(
                  thumObj.thumbDir,
                  thumObj.uuid.toString() + "_" + size.toString()
                )
              );
                ...
                .stream(function (err, stdout, stderr) {
                  stdout
                    ...
                    .on("error", function (error) {
                      console.log(
                        "Error in making thumbnail " + thumObj.uuid + "_" + size
                      );
                      callbackThumb(error, undefined);
                    })
                    .on("finish", function () {
    ...
    logger.error(
      "[DVP-FIleService.LocalThumbnailMaker] - [%s] - Exception in operation of local thumbnail creation of %s ",
      thumObj.reqId,
      thumObj.uuid
    );
...
function MongoFileUploader(dataObj, callback) {
  try {
        ...
        if (dataObj.encNeeded) {
          ...
          uploadReadStream
            ...
            .on("error", function (error) {
              ...
              cipher.end();
              logger.error(
                "[DVP-FIleService.MongoFileUploader] - [%s]  - Error in Encripted stream uploading to DB ",
                dataObj.reqId,
                error
              );
              db.close();
            ...
            .on("finish", function () {
              logger.info(
                "[DVP-FIleService.MongoFileUploader] - [%s]  - Encripted and uploaded to Mongo successfully ",
                dataObj.reqId
              );
              cipher.end();
              ...
              if (fileStruct == "image") {
                sizeArray.forEach(function (size) {
                  thumbnailArray.push(function createContact(callbackThumb) {
                    gm(fs.createReadStream(dataObj.Fobj.path))
                      .resize(size, size)
                      .quality(50)
                      .stream(function (err, stdout, stderr) {
                        var writeStream = ThumbBucket.openUploadStream(
                          dataObj.rand2 + "_" + size
                        );
                        stdout
                          .pipe(writeStream)
                          .on("error", function (error) {
                            logger.error(
                              "[DVP-FIleService.MongoFileUploader] - [%s]  - Error in making thumbnail %s _ %s",
                              dataObj.reqId,
                              dataObj.rand2,
                              size,
                              error
                          ...
                          .on("finish", function (thumb) {
                            logger.info(
                              "[DVP-FIleService.MongoFileUploader] - [%s]  - Successfully created thumbnail %s _ %s",
                              dataObj.reqId,
                              dataObj.rand2,
                              size
                            );
          ...
          uploadReadStream
            ...
            .on("error", function (error) {
              logger.error(
                "[DVP-FIleService.MongoFileUploader] - [%s]  - Error in file uploading to DB ",
                dataObj.reqId,
                error
              );
              db.close();
            ...
            .on("finish", function () {
              logger.info(
                "[DVP-FIleService.MongoFileUploader] - [%s]  - File uploaded to Mongo successfully ",
                dataObj.reqId
              );
              //RedisPublisher.updateFileStorageRecord(dataObj.Category, dataObj.Fobj.sizeInMB, dataObj.cmp, dataObj.ten);
    ...
    logger.error(
      "[DVP-FIleService.MongoFileUploader] - [%s]  - Error in operation ",
      dataObj.reqId
    );
                  ...
                  try {
                    DbConn.FileUpload.find({
                      where: [
                        ...
                        { CompanyId: resFile.CompanyId },
                        { TenantId: resFile.TenantId },
                        { ApplicationId: appID },
                      ],
                    })
                      .then(function (resVFileNm) {
                        if (!resVFileNm) {
                          resFile
                            .setApplication(resApp)
...
function localStoreHandler(fileData, callback) {
  try {
    ...
    var newDir = path.join(
      uploadPath,
      "Company_" +
        fileData.cmp.toString() +
        "_Tenant_" +
        fileData.ten.toString(),
      file_category,
    ...
    var thumbDir = path.join(
      ...
      "Company_" +
        ...
        "_Tenant_" +
        fileData.ten.toString(),
      file_category + "_thumb",
      year.toString() + "-" + month.toString() + "-" + date.toString()
    );

    logger.debug(`The file path is ${newDir}`);
    logger.debug(`The thumb path is ${thumbDir}`);
    ...
    mkdirp(newDir, function (err) {
      if (err) {
        logger.info(
          "[DVP-FIleService.DeveloperUploadFiles] - [%s] - [PGSQL] - Failed to lode specific location to save",
          fileData.reqId
        );
        callback(err, undefined);
      } else {
        console.log(
          "Uploading path : " + path.join(newDir, fileData.rand2.toString())
        );
        ...
        if (fileData.encNeeded) {
          ...
          fs.createReadStream(fileData.Fobj.path)
            .pipe(cipher)
            .pipe(
              fs.createWriteStream(path.join(newDir, fileData.rand2.toString()))
            )
            .on("error", function (error) {
              cipher.end();
              console.log("Error in piping and encrypting");
              logger.info(
                "[DVP-FIleService.DeveloperUploadFiles] - [%s] - [PGSQL] - Error in piping and encrypting",
                fileData.reqId
              );
              callback(error, undefined);
            })
            .on("finish", function () {
              ...
              fileData.Fobj.thumbDir = thumbDir;
              logger.info(
                "[DVP-FIleService.DeveloperUploadFiles] - [%s] - [PGSQL] - File  encrypted and stored",
                fileData.reqId
              );
          ...
          fs.createReadStream(fileData.Fobj.path)
            ...
            .on("error", function (error) {
              ...
              logger.info(
                "[DVP-FIleService.DeveloperUploadFiles] - [%s] - [PGSQL] - Error in piping",
                fileData.reqId
              );
              callback(error, undefined);
            })
            .on("finish", function () {
              ...
              fileData.Fobj.thumbDir = thumbDir;
              logger.info(
                "[DVP-FIleService.DeveloperUploadFiles] - [%s] - [PGSQL] - File stored",
                fileData.reqId
              );
              callback(undefined, fileData.Fobj);
            });
    ...
    logger.info(
      `[DVP-FIleService.DeveloperUploadFiles] - [${fileData.reqId}] - [HTTP] - Exception occurred while uploading files to LOCAL - ${e.message}`
    );
    callback(e, undefined);
  }
}
...
function localStorageRecordHandler(dataObj, callback) {
  try {
    localStoreHandler(dataObj, function (errStore, resStore) {
      if (errStore) {
        logger.info(
          "[DVP-FIleService.DeveloperUploadFiles.localStorageRecordHandler] - [%s] - [PGSQL] - Failed to lod specific location to save",
          dataObj.reqId
        );
        ...
        RedisPublisher.updateFileStorageRecord(
          resStore.Category,
          resStore.sizeInKB,
          dataObj.cmp,
          dataObj.ten
        ...
        removeSourceEmptyRecord(dataObj.cmp, dataObj.ten, dataObj.resvID);
        logger.info(
          "[DVP-FIleService.DeveloperUploadFiles.localStorageRecordHandler] - [%s] - [PGSQL] - Error in operation",
          dataObj.reqId
        );
    ...
    removeSourceEmptyRecord(dataObj.cmp, dataObj.ten, dataObj.resvID);
    logger.info(
      "[DVP-FIleService.DeveloperUploadFiles.localStorageRecordHandler] - [%s] - [HTTP] - Exception occurred",
      dataObj.reqId
    );
...
function searchFileCategory(category, company, tenant, reqId, callback) {
  try {
    if (category) {
      ...
      DbConn.FileCategory.findOne({
        ...
        .then(function (resCat) {
          if (resCat) {
            logger.info(
              "[DVP-FIleService.searchFileCategory] - [%s] - [PGSQL] - File Category found : %s",
              reqId,
              category
            );
            ...
            logger.error(
              "[DVP-FIleService.searchFileCategory] - [%s] - [PGSQL] - Error in searching file Category : %s",
              reqId,
              category
            );
      ...
      logger.error(
        "[DVP-FIleService.searchFileCategory] - [%s] - [HTTP] - No category found",
        reqId
      );
    ...
    logger.error(
      "[DVP-FIleService.searchFileCategory] - [%s] - [HTTP] - Exception occurred",
      reqId
    );
    callback(e, undefined);
  }
...
function DeveloperUploadFiles(fileObj, callback) {
  try {
    ...
    if (fileObj.Fobj) {
      ...
      searchFileCategory(
        ...
        function (errCat, resCat) {
          if (errCat) {
            logger.error(
              "[DVP-FIleService.DeveloperUploadFiles] - [%s]  - Error in checking file categories ",
              fileObj.reqId
            );
            ...
            checkOrganizationSpaceLimit(
              fileObj.cmp,
              fileObj.ten,
              fileObj.Fobj.sizeInKB,
              fileObj.Category,
              function (errCheck, resCheck) {
                if (errCheck) {
                  callback(errCheck, undefined, fileObj.tempPath);
                } else {
                  if (fileStore == "MONGO") {
                    logger.info(
                      "[DVP-FIleService.DeveloperUploadFiles] - [%s]  - New attachment on process of uploading to MongoDB",
                      fileObj.reqId
                    );
      ...
      logger.error(
        "[DVP-FIleService.DeveloperUploadFiles] - [%s] - No file object found ",
        fileObj.reqId
      );
    ...
    logger.error(
      "[DVP-FIleService.DeveloperUploadFiles] - [%s] - Exception occurred when new attachment object saving starting ",
      fileObj.reqId,
      ex
    );
...
function GetUploadedFileSize(company, tenant, callback) {
  DbConn.FileUpload.sum("Size", {
    where: [
      { CompanyId: company },
      { TenantId: tenant },
      { ObjCategory: { $ne: "CONVERSATION" } },
    ],
  })
    .then(function (resSum) {
...
function removeSourceEmptyRecord(company, tenant, recId) {
  DbConn.FileCategory.destroy({
    ...
    .then(function (resCat) {
      if (resCat) {
        logger.info(
          "[DVP-FIleService.removeSourceEmptyRecord] - [%s] - [PGSQL] - File removed : %s",
          reqId
        );
      } else {
        logger.info(
          "[DVP-FIleService.removeSourceEmptyRecord] - [%s] - [PGSQL] - No such file to remove",
          reqId
        );
      }
    })
    .catch(function (errCat) {
      logger.error(
        "[DVP-FIleService.removeSourceEmptyRecord] - [%s] - [PGSQL] - Error in removing file",
        reqId
      );
    });
}
...
function DeveloperReserveFiles(
  Display,
  fileName,
  rand2,
  cmp,
...
function recordFileDetails(dataObj, callback) {
  try {
    ...
    if (dataObj.resvID && dataObj.Fobj) {
      ...
      DbConn.FileUpload.update(
        ...
        .then(function (resUpdate) {
          logger.info(
            "[DVP-FIleService.recordFileDetails] - [%s] - Uploaded file record Status Set to UPLOADED ",
            dataObj.reqId
          );
        ...
        .catch(function (errUpdate) {
          logger.error(
            "[DVP-FIleService.recordFileDetails] - [%s] - Error in Status update of reserved File record  ",
            dataObj.reqId
          );
            ...
            try {
              ...
              NewUploadObj.save()
                .then(function (resUpFile) {
                  if (resUpFile) {
                    logger.info(
                      "[DVP-FIleService.recordFileDetails] - [%s] - [PGSQL] - New attachment ID %d successfully inserted",
                      dataObj.reqId,
                      dataObj.rand2
                    );
                    searchFileCategory(
                      ...
                      function (errCat, resCat) {
                        if (errCat) {
                          ...
                          resUpFile
                            ...
                            .then(function (resCatset) {
                              logger.info(
                                "[DVP-FIleService.recordFileDetails] - [%s] - [PGSQL] - Attached category id %d to File %s successfully",
                                dataObj.reqId,
                                resCat.id,
                                dataObj.rand2
                              );
                            ...
                            .catch(function (errCatSet) {
                              logger.error(
                                "[DVP-FIleService.recordFileDetails] - [%s] - [PGSQL] - Failed to Attach category id %d to File %s successfully",
                                dataObj.reqId,
                                resCat.id,
                                dataObj.rand2
                              );
                    ...
                    logger.info(
                      "[DVP-FIleService.recordFileDetails] - [%s] - [PGSQL] - New attachment ID %s failed to save",
                      dataObj.reqId,
                      dataObj.rand2
                    );
                    callback(
                      new Error("Upload records saving failed"),
                      undefined
                    );
                  }
                })
                .catch(function (errUpFile) {
                  logger.error(
                    "[DVP-FIleService.recordFileDetails] - [%s] - [PGSQL] - New attachment ID %s insertion failed / Error",
                    dataObj.reqId,
                    dataObj.rand2
                  );
                  callback(errUpFile, undefined);
              ...
              logger.error(
                "[DVP-FIleService.recordFileDetails] - [%s] - Exception occurred when new attachment object creating ",
                dataObj.reqId,
                ex
              );
              callback(ex, undefined);
    ...
    logger.error(
      "[DVP-FIleService.recordFileDetails] - [%s] - Exception occurred when method executing ",
      dataObj.reqId,
      e
    );
    callback(e, undefined);
...
function checkOrganizationSpaceLimit(
  company,
  tenant,
  newFileSize,
  category,
